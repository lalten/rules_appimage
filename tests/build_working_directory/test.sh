...
env="$(env -i -- PATH="/bin:/usr/bin" tests/build_working_directory/test.appimage)"
grep -q OWD= <<<"$env"
...
env="$(env -i -- PATH="/bin:/usr/bin" APPIMAGE_EXTRACT_AND_RUN=1 tests/build_working_directory/test.appimage)"
